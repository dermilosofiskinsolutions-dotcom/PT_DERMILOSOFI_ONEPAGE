...
<head>
    ...
    <title>Edit Blog - PT. DERMILOSOFI</title>
        ...
        <div class="admin-card">
            ...
            <form action="{{ route('blogs.update', $blog) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-3">
                    ...
                    @if($blog->image_path)
                        <small class="form-text text-muted">Current image: {{ basename($blog->image_path) }}</small>
                    @endif
                </div>

                <div class="mb-3">
                    <label for="published_date" class="form-label">Published Date</label>
                    <input type="date" class="form-control" id="published_date" name="published_date" value="{{ $blog->published_date ? $blog->published_date->format('Y-m-d') : '' }}">
                </div>

                <div class="mb-3">
                    <label for="tags" class="form-label">Tags</label>
                    <input type="text" class="form-control" id="tags" name="tags" value="{{ $blog->tags }}" placeholder="Separate tags with commas">
                </div>
